

        <div class="super_container">

<!-- start Header -->

<header class="header trans_400" id="headerlogo">
    <div class="header_content d-flex flex-row align-items-center jusity-content-start trans_400">

        <!-- Logo -->
        <div class="logo">
            <a href="index.html">
                <div><img src="../images/logoheader.png" title="medical tecnology logo"></div>
            </a>
        </div>

        <!-- Main Navigation -->
        <nav class="main_nav">
            <ul class="d-flex flex-row align-items-center justify-content-start">
                <li><a href="index.php">Home</a></li>
                <li><a href="corona.php">Health awareness</a></li>
                <li><a href="index.php#BestCare">Best Care</a></li>
                <li><a href="index.php#whych">Why choose us?</a></li>
                <li><a href="index.php#Services">Services</a></li>
                <li><a href="index.php#offers">Offers</a></li>
                <li><a href="#Contactus">Contact us</a></li>

            </ul>
        </nav>

        <div class="header_extra d-flex flex-row align-items-center justify-content-end ml-auto">
        <!-- Hamburger main icon display when minimize size of screen  -->
            <div class="hamburger"><i class="fa fa-bars" aria-hidden="true"></i></div>
        </div>

    </div>
</header>

<div class="container-booking">

<div class="row">
    <div class="col-6 left">
        <div class="left-content">
            <div class="row">
                <h6 id='' >Patiant Name <span>Omar Reda</span></h6>            
            </div>
            <div class="row">
                <h6>Register Date <span id='date-register'></span></h6>            
            </div>
            <div class="row" style="border-bottom: none;">
                <h6 class = "countform-register" id="">
                    Time remaining <span id = 'counter-register' style="padding-left: 42px"></span></h6>                                
            </div>
            <div class="row btn-Cancel" id="cancel-register" style="border-bottom: none;">
               <button type="button" class="btn btn-danger">Cancel appointment</button>
            </div>
            <div class="row btn-start" id="startRegister" style="border-bottom: none;">
              <button type="button" class="btn btn-success btn-s">Start</button>
           </div>
         </div>
    </div>
    <div class="col-6 right">
      <div class="row right-content">
          <h4>Your appointment is confirm</h4>            
      </div>
      <div class="row">
         <div class="right-doc">
           <div class="row">
                 <h6>Doctor <span>Amr mohammed</span></h6>            
          </div>
          <div class="row">
              <h6>Phone <span>+020112158746</span></h6>            
           </div>
          <div class="row">
               <h6>Address <span style="padding-left: 72px;">1481 Creekside Lane Avila Beach, CA 931 </span></h6>            
          </div>         
       </div>
     </div>
    </div>
  </div>
</div>


